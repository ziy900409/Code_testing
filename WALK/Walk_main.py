...
# operation: '+' 或 '-'
COL_OPERATIONS = [
    {'new_col_name': '總收益', 'col1': '銷售額', 'col2': '其他收入', 'operation': '+'},
    {'new_col_name': '淨利潤', 'col1': '總收益', 'col2': '總成本', 'operation': '-'},
    # 您可以新增更多運算
]

"FP time", "SI time(s)", "plate 1 COPX analog", "plate 1 COPY analog",
"plate 2 COPX analog","plate 2 COPY analog", "left_cop_x(mm)", "left_cop_y(mm)",
"right_cop_x(mm)", "right_cop_y(mm)"




# 定義要計算相關係數的欄位 (可以是原始欄位或運算後的新欄位)
COLS_FOR_CORRELATION = ['銷售額', '其他收入', '總成本', '總收益', '淨利潤', '廣告費']
...
COLS_TO_PLOT = [
    {'x_col': '廣告費', 'y_col': '銷售額', 'plot_type': 'scatter', 'title_suffix': '廣告費與銷售額關係'},
    {'x_col': '月份', 'y_col': '淨利潤', 'plot_type': 'line', 'title_suffix': '每月淨利潤趨勢'}, # 假設有'月份'欄位
    # 您可以新增更多繪圖設定
]
# --- 設定結束 ---
...
PLOTS_CONFIG = {
    ...
    "right 左右": {
        ...
        "title": "不同位置的壓力比較"
        # 如果省略 xlabel/ylabel，將使用實際欄位名或通用標籤
    }
    # ...更多子圖設定...
}
...
# %%
# 在 process_file 函數中，讀取完 df 後：
excel_path = r"C:\Users\User\Downloads\範例檔-20250519T142424Z-1-001\範例檔\S1皮爾森相關分析範例\S1_GOLF_1 FP&SI extra v2.xlsx"
df2 = df.iloc[:, :10]


plot_filepath = create_custom_subplots(df_1, PLOTS_CONFIG, col_name_map, OUTPUT_DIRECTORY, original_filename_base="123")

# %%

def process_file(excel_path, original_filename):
    """
    ...
    # 6. 針對某些欄位進行繪圖，並輸出圖片
    print("\n進行繪圖...")
    base_filename_for_plot = os.path.splitext(original_filename)[0]
    for i, plot_info in enumerate(COLS_TO_PLOT):
        x_col = plot_info['x_col']
        y_col = plot_info['y_col']
        plot_type = plot_info.get('plot_type', 'scatter') # 預設為散佈圖
        title_suffix = plot_info.get('title_suffix', f'{x_col}_vs_{y_col}')

        if x_col in df.columns and y_col in df.columns:
            if not (pd.api.types.is_numeric_dtype(df[x_col]) and pd.api.types.is_numeric_dtype(df[y_col])) and plot_type != 'line': # line plot can handle non-numeric x-axis for index-like plotting
                 if not (pd.api.types.is_numeric_dtype(df[y_col])): # For line plot, y must be numeric
                    print(f"  警告: 欄位 '{x_col}' 或 '{y_col}' 非數值型，無法繪製 '{plot_type}' 圖。跳過。")
                    continue
            ...
            if plot_type == 'scatter':
                ...
                # 如果X軸是類別或日期型，排序可能有助於視覺化
                # df_sorted = df.sort_values(by=x_col) if x_col in df.columns else df
                # sns.lineplot(data=df_sorted, x=x_col, y=y_col, marker='o')
                sns.lineplot(data=df, x=x_col, y=y_col, marker='o') # marker 可選
            else:
                print(f"  警告: 不支援的圖表類型 '{plot_type}'。跳過。")
                plt.close() # 關閉未使用的圖表
                continue

            plot_title = f"{base_filename_for_plot}: {title_suffix}"
            plt.title(plot_title)
            plt.xlabel(x_col)
            plt.ylabel(y_col)
            plt.grid(True) # 加入網格線

            plot_output_filename = f"{base_filename_for_plot}_plot_{i+1}_{x_col}_vs_{y_col}.png"
            plot_output_path = os.path.join(OUTPUT_DIRECTORY, plot_output_filename)
            try:
                plt.savefig(plot_output_path)
                print(f"  圖表已儲存至: {plot_output_path}")
            except Exception as e:
                print(f"  儲存圖表時發生錯誤: {e}")
            plt.close() # 關閉當前圖表，避免後續圖表重疊
        else:
            missing_plot_cols = []
            if x_col not in df.columns:
                missing_plot_cols.append(x_col)
            if y_col not in df.columns:
                missing_plot_cols.append(y_col)
            print(f"  警告: 繪圖欄位 {', '.join(missing_plot_cols)} 不存在。跳過此圖表。")
    print(f"--- 完成處理檔案: {original_filename} ---\n")
...
if __name__ == "__main__":
    ...
    if not os.path.isdir(INPUT_DIRECTORY):
        print(f"錯誤: 輸入資料夾 '{INPUT_DIRECTORY}' 不存在。請檢查路徑。")
        # 為了方便測試，如果INPUT_DIRECTORY不存在，可以提示使用者建立範例檔案
        # 例如，建立一個名為 `sample_report_data.xlsx` 的檔案
        # 包含欄位: '月份', '銷售額', '其他收入', '總成本', '廣告費'
        # 並填入一些數值資料
        exit()
    ...
    for filename in os.listdir(INPUT_DIRECTORY):
        # 1. 讀取檔案路徑，並判斷相關字串是否包含在檔名裡面
        if FILENAME_KEYWORD.lower() in filename.lower() and (filename.endswith('.xlsx') or filename.endswith('.xls')):
            file_path = os.path.join(INPUT_DIRECTORY, filename)
            process_file(file_path, filename)
            found_files_count += 1

    if found_files_count == 0:
        print(f"在 '{INPUT_DIRECTORY}' 中沒有找到包含關鍵字 '{FILENAME_KEYWORD}' 的 Excel 檔案。")
    else:
        print(f"全部處理完成，共處理了 {found_files_count} 個檔案。")