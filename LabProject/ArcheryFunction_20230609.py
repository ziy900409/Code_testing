...
# 帶通濾波頻率
bandpass_cutoff = [8/0.802, 400/0.802]
# 低通濾波頻率
...
# %% EMG data processing
def EMG_processing(raw_data, release_time=None):
    '''
    Parameters
    ----------
    cvs_file_list : str
    ...
    # 測試用
    # raw_data = pd.read_csv(r"E:\python\EMG_Data\Shooting_data_20230617\202305 Shooting\Raw_Data\\\A2\MVC\A2_MVC_Rep_7.6_QL.csv",
    #                     encoding='UTF-8')
    # 1.  -------------前處理---------------------------
    # 1.1.-------------計算所有sensor之採樣頻率----------
    ...
    for col in range(len(num_columns)):
        ...
        Fs.append(int((len(data_time) - (raw_data.iloc[:, num_columns[col]][::-1] != 0).argmax(axis = 0))/freq*1000))
        # 取截止時間
        all_stop_time.append(raw_data.iloc[(len(data_time) - (raw_data.iloc[:, num_columns[col]][::-1] != 0).argmax(axis = 0))-1 ,
                                       num_columns[col]-1])
    # 1.2.-------------計算平均截止時間------------------
    # 丟棄NAN的值，並選擇最小值
    ...
    for col in range(len(num_columns)):
        ...
        if isnan[0].size == 0:
        # 計算Bandpass filter
            bandpass_sos = signal.butter(1, bandpass_cutoff,  btype='bandpass', fs=sample_freq, output='sos')
        ...
        # 設定給斷訊超過 0.1 秒的 sensor 警告
            ...
            bandpass_sos = signal.butter(1, bandpass_cutoff,  btype='bandpass', fs=sample_freq, output='sos')
            bandpass_filtered = signal.sosfiltfilt(bandpass_sos,
                                                   raw_data.iloc[:(np.shape(raw_data)[0] - data_len[col]), num_columns[col]].fillna(0))
