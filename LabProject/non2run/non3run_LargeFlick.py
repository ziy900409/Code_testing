# -*- coding: utf-8 -*-
"""
Created on Mon Apr  7 15:36:50 2025
...
import gen_function_001 as func

# func.read_c3d(c3d_list[num])
...
RawData_folder = "1.Motion\\"
processingData_folder = "4.ProcessData\\"

save_place_list = [
    "\\1. SmallFlick\\", "\\2. SmallTrack\\", "\\3. SpiderShot\\",
    # "\\4. GridShot\\"
    ]
task_name_list = [
    "SmallFlick", "SmallTrack", "LargeFlick",
    # "GridShot"
    ]

# task_args_list = list(zip(save_place_list, task_name_list))
EMG_folder = "3.EMG\\"
...
stage_file_path = r"D:\BenQ_Project\01_UR_lab\2025_02 Asymmetry\1.Motion\2.Cortex\ZowieAsymmetry3run_StagingFile_0327.xlsx"
...
# def process_subject_folder(args):
#     save_place, task_name = args 
for save_num in range(len(save_place_list)):
    save_place = save_place_list[save_num]
    task_name = task_name_list[save_num]
    all_hand_include_angle = pd.DataFrame({}, columns=['mouse'])
    all_slope_data = pd.DataFrame(
        {}, columns=['subject', 'data_name', 'mouse'] + muscle_name)
    all_imvc_data = pd.DataFrame(
        {}, columns=['subject', 'data_name', 'mouse', 'method'] + muscle_name)
    
    all_emg_slope_data = pd.DataFrame(
        {}, columns=['data_name', 'mouse'] + muscle_name)
    # 創建資料貯存空間
    ...
    for folder_name in vicon_folder:
        # 讀資料夾下的 c3d file
        c3d_folder_path = folder_path + RawData_folder + "1.Vicon\\" + folder_name
        # 讀取子資料夾
        ...
        for num in range(len(c3d_list)):
            ...
            if "tpose_elbow" in c3d_list[num].lower():
                print(c3d_list[num])
                motion_info, motion_data, analog_info, FP_data, np_motion_data = func.read_c3d(
                    c3d_list[num], method='vicon')
                # 0. 取代欄位名稱
                motion_data.rename(columns=lambda x: x.replace(
                    'elbow:', ''), inplace=True)
                # 1. 設定輸入計算 Virtual marker 參數 : 手肘內上髁, 外上髁, UA1, UA3
                R_Elbow_Med = motion_data.loc[:, "R.Elbow.Med_x":"R.Elbow.Med_z"].dropna(
                    axis=0)
                R_Elbow_Lat = motion_data.loc[:, "R.Elbow.Lat_x":"R.Elbow.Lat_z"].dropna(
                    axis=0)
                ...
                ind_frame = min([np.shape(R_Elbow_Med)[0], np.shape(
                    R_Elbow_Lat)[0], np.shape(UA1)[0], np.shape(UA3)[0]])
                ...
                for frame in ind_frame:
                    p1_all.iloc[frame:] = (kincal.transformation_matrix(R_Elbow_Lat.iloc[frame, :].values, UA1.iloc[frame, :].values, UA3.iloc[frame, :].values,
                                                                        R_Elbow_Med.iloc[frame, :].values, np.array(
                                                                            [0, 0, 0]),
                                                                        rotation='GCStoLCS'))
                    # 4. 清除不需要的變數
        ...
        for num in range(len(motion_list)):
            ...
            # 0. 處理檔名問題------------------------------------------------------
            save_name, extension = os.path.splitext(
                motion_list[num].split('\\', -1)[-1])
            ...
            # 取代欄位名稱
            motion_data.rename(columns=lambda x: x.replace(
                str(folder_name + ':'), ''), inplace=True)
            motion_data.rename(columns=lambda x: x.replace(
                'mouse:', ''), inplace=True)
            motion_data = motion_data.fillna(0)
            # 取代 LABELS 中的 subject name
            new_columns_list = [s.replace(str(folder_name + ':'), '')
                                for s in motion_info["LABELS"]]
            ...
            task_start = 0
            # 結束索引是開始後加15秒
            task_end = task_start + int(motion_info['frame_rate']*end_time)
    
            # 設定 sampling rate
            sampling_time = 1/motion_info['frame_rate']
            # 前處理 motion data, 進行低通濾波
            lowpass_sos = signal.butter(
                2, lowpass_cutoff,  btype='low', fs=motion_info['frame_rate'], output='sos')
            ...
            mouse_mean = filted_motion.loc[task_start:task_end, ['M3_x', 'M3_y', 'M3_z']].values - \
                filted_motion.loc[task_start:task_end, [
                    'R.M.Finger1_x', 'R.M.Finger1_y', 'R.M.Finger1_z']].values
    
            distance = np.linalg.norm(filted_motion.loc[task_start:task_end, ['M3_x', 'M3_y', 'M3_z']].values -
                                      filted_motion.loc[task_start:task_end, [
                                          'R.M.Finger1_x', 'R.M.Finger1_y', 'R.M.Finger1_z']].values,
                                      axis=1)
            # 單獨拉出 x, y 軸位置
            ...
            R_Thumb2 = filted_motion.loc[:, ['R.Thumb2_x', 'R.Thumb2_y', 'R.Thumb2_z']].\
                iloc[task_start:task_end, :].values
            R_Thumb3 = filted_motion.loc[:, ['R.Thumb3_x', 'R.Thumb3_y', 'R.Thumb3_z']].\
                iloc[task_start:task_end, :].values
            ...
            ThumbFinger = kincal.included_angle(R_Thumb2,
                                                R_Thumb1,
                                                wrist_mid)
            ThumbFinger2 = kincal.included_angle(R_Thumb3,
                                                  R_Thumb2,
                                                  R_Thumb1)
            # 2.1. 儲存資料-----------------------------------------------------------
            # 將資料儲存至矩陣
            hand_include_angle = pd.DataFrame({"file_name": save_name,
                                              "subject": trial_info["subject"],
                                               ...
                                               "Thumb_angle_std": np.std(ThumbFinger),
                                                "Thumb2_angle_mean": np.mean(ThumbFinger2),
                                                "Thumb2_angle_min": np.min(ThumbFinger2),
                                                "Thumb2_angle_max": np.max(ThumbFinger2),
                                                "Thumb2_angle_std": np.std(ThumbFinger2)
                                               },
                                              index=[0])
            ...
            # 計算大臂, 小臂, 手掌隨時間變化的坐標系
            # motion_info: new
            ArmCoord, ForearmCoord, HandCoord, new_motion_info, new_motion_data = kincal.UpperExtremty_coord(
                trun_motion_np, motion_info, p1_all)
            # 刪除不需要的變數
            del motion_data, analog_data, np_motion_data
            # --------------------------------------------------------------------
            # 計算手指的關節角度: 只計算食、中指以及大拇指.
            tem_hand_angle_table = kincal.finger_angle_cal(
                motion_list[num], new_motion_data, new_motion_info)
            ...
            hand_angle_table = pd.concat([hand_angle_table, tem_hand_angle_table],
                                         ignore_index=True)
            # 計算關節的旋轉矩陣
            ElbowRot = kincal.joint_angle_rot(
                ArmCoord, ForearmCoord, OffsetRotP=static_ArmCoord, OffsetRotD=static_ForearmCoord)
            WristRot = kincal.joint_angle_rot(
                ForearmCoord, HandCoord, OffsetRotP=static_ForearmCoord, OffsetRotD=static_HandCoord)
            ElbowEuler = kincal.Rot2EulerAngle(ElbowRot, "zyx")
            WristEuler = kincal.Rot2EulerAngle(WristRot, "zxy")
            # 使用旋轉矩陣轉換成毆拉參數後再計算角速度與角加速度
            Elbow_AngVel, Elbow_AngAcc = kincal.Rot2LocalAngularEP(
                ElbowRot, 180, place="joint", unit="degree")
            Wrist_AngVel, Wrist_AngAcc = kincal.Rot2LocalAngularEP(
                WristRot, 180, place="joint", unit="degree")
            ...
            emg_save_path = folder_path + processingData_folder + folder_name + "\\" + \
                save_place + "\\2. EMG\\"
            # 前處理EMG data
            processing_data, bandpass_filtered_data = emg.EMG_processing(
                emg_path, smoothing=smoothing)
            # 擷取 EMG 資料.
            emg_fs = 1 / (processing_data.iloc[1, 0] - processing_data.iloc[0, 0])
    
            emg_task_start = int(task_start * emg_fs / motion_info['frame_rate'])
            emg_task_end = int(task_end * emg_fs / motion_info['frame_rate'])
            processing_data = processing_data.iloc[emg_task_start:emg_task_end, :].reset_index(
                drop=True)
            bandpass_filtered_data = bandpass_filtered_data.iloc[task_start:task_end, :]
            # 畫 bandpass 後之資料圖
            ...
            # import pdb
            # pdb.set_trace()
            vel_motion_table, vel_emg_table = FigPlot.plot_arm_angular(motion_list[num], str(folder_name + save_place),
                                                                       Elbow_AngVel, Wrist_AngVel, emg_iMVC,
                                                                       joint_threshold, cal_method='vel')
            acc_motion_table, acc_emg_table = FigPlot.plot_arm_angular(motion_list[num], str(folder_name + save_place),
                                                                       Elbow_AngAcc, Wrist_AngAcc, emg_iMVC,
                                                                       joint_threshold, cal_method='acc')
    
            # 合併統計資料 table
            # motion
            all_motion_data_table = pd.concat(
                [all_motion_data_table, vel_motion_table])
            all_motion_data_table = pd.concat(
                [all_motion_data_table, acc_motion_table])
            # emg
    ...
    # 儲存檔案
    with pd.ExcelWriter(folder_path + "4. Statistics\\" + save_place + "All_" + task_name + "_vicon_motion+emg_" + formatted_date + ".xlsx",
                        engine='openpyxl') as writer:
        all_emg_data_table.to_excel(writer, sheet_name='arm_emg_motion', index=False, header=True)
        all_motion_data_table.to_excel(writer, sheet_name='emg_table', index=False, header=True)
    ...
    # save_place, task_name = args
    # print(f"🚀 開始處理任務：{task_name} - {save_place}")
    # # 你原本的任務處理程式碼放這邊（省略）
...
if __name__ == "__main__":
    ...
    with Pool(processes=cpu_count()) as pool:
        # tqdm 包住 imap_unordered 以顯示進度條
        results = list(tqdm(pool.imap_unordered(process_subject_folder, task_args_list),
                            total=len(task_args_list),
                            desc="處理進度",
                            ncols=100))  # 你可以調整 ncols 改變寬度
...
for save_num in range(len(save_place_list)):
    ...
    for folder_name in cortex_folder:
        # 讀資料夾下的 c3d file
        c3d_folder_path = folder_path + RawData_folder + "2.Cortex\\" + folder_name
        # 讀取只包含 .c3d 與 MVC 的資料夾路徑
        c3d_list = func.Read_File(c3d_folder_path, ".c3d")
        ...
        for num in range(len(c3d_list)):
            ...
            if "tpose_elbow" in c3d_list[num].lower():
                print(c3d_list[num])
                motion_info, motion_data, analog_info, FP_data, np_motion_data = func.read_c3d(
                    c3d_list[num], method='cortex')
                # 0. 取代欄位名稱
                motion_data.rename(columns=lambda x: x.replace(
                    'EC2 Wight_Elbow:', ''), inplace=True)
                # 1. 設定輸入計算 Virtual marker 參數 : 手肘內上髁, 外上髁, UA1, UA3
                # 1.1. 新增 Virtual marker
                if "V_Elbow_Med_x" in motion_data.columns:
                    R_Elbow_Med = motion_data.loc[:, "V_Elbow_Med_x":"V_Elbow_Med_z"].dropna(
                        axis=0)
                else:
                    R_Elbow_Med = motion_data.loc[:, "R.Elbow.Med_x":"R.Elbow.Med_z"].dropna(
                        axis=0)
        ...
        motion_list = [file for file in c3d_list if task_name in file]
        stage_file_list = [
            file for file in stage_file["Motion_File_C3D"].dropna() if task_name in file]
        # 只留下
        motion_list = [file for file in motion_list if any(stage in file for stage in stage_file_list)]
        for stage_num in range(len(stage_file["Motion_File_C3D"])):
            for num in range(len(motion_list)):
                if str(stage_file["Motion_File_C3D"][stage_num]) in motion_list[num]:
                    ...
                    motion_info, motion_data, analog_info, analog_data, np_motion_data = func.read_c3d(r'D:\\BenQ_Project\\01_UR_lab\\2025_02 Asymmetry\\\\1.Motion\\2.Cortex\\S12\\S04_GridShot_DA_01_ok.c3d',
                                                                                                       method='cortex')
                    # 取代欄位名稱
                    motion_data.rename(columns=lambda x: x.replace(
                        str("EC2 Wight" + ':'), ''), inplace=True)
                    motion_data.rename(columns=lambda x: x.replace(
                        'mouse:', ''), inplace=True)
                    motion_data.rename(columns=lambda x: x.replace(
                        'Mouse:', ''), inplace=True)
                    ...
                    # 取代 LABELS 中的 subject name
                    new_columns_list = [
                        s.replace(str("EC2 Wight" + ':'), '') for s in motion_info["LABELS"]]
                    new_columns_list = [s.replace('Mouse:', '')
                                        for s in new_columns_list]
                    motion_info["LABELS"] = new_columns_list
                    # 設定開始索引
                    ...
                    task_start = int(stage_file["start"][stage_num] + motion_info['frame_rate']*start_time)
                    ...
                    # 3. 疲勞分析 -----------------------------------------------------------
                    # 定義及設定存檔路徑
                    emg_path = str(folder_path + EMG_folder + folder_name + "\\" + stage_file['EMG_File'][stage_num])
                    emg_save_path = folder_path + processingData_folder + folder_name + "\\" + \
                        save_place + "\\2. EMG\\"
                    # 前處理EMG data
                    ...
                    # 擷取 EMG 資料.
                    emg_fs = 1 / \
                        (processing_data.iloc[1, 0] - processing_data.iloc[0, 0])
        
                    emg_task_start = int(start_time * emg_fs)
                    emg_task_end = emg_task_start + int(end_time * emg_fs)
                    # truncut data
                    processing_data = processing_data.iloc[emg_task_start:emg_task_end, :].reset_index(
                        drop=True)
                    bandpass_filtered_data = bandpass_filtered_data.iloc[emg_task_start:emg_task_end, :]
                    # 畫 bandpass 後之資料圖
                    emg.plot_plot(bandpass_filtered_data, str(emg_save_path),
                                  save_name, "_Bandpass")
                    # 畫smoothing 後之資料圖
                    ...
                    emg_iMVC.iloc[:, 1:] = np.divide(abs(processing_data.iloc[:, 1:].values),
                                                     MVC_value.values)*100
                    # truncut iMVC data
                    emg_iMVC = emg_iMVC.iloc[emg_task_start:emg_task_end, :]
                    pd.DataFrame(emg_iMVC).to_excel(emg_save_path + save_name + "_iMVC.xlsx",
                                                    sheet_name='Sheet1', index=False, header=True)
                    # 進行中頻率分析
                    med_freq_data, slope_data = emg.median_frquency(emg_path,
                                                                    duration, emg_save_path, save_name,
                                                                    begin=[emg_task_start, emg_task_end])
                    # 儲存斜率的資料，並合併成一個資料表
                    slope_data['data_name'] = [save_name]
                    slope_data["subject"] = [trial_info["subject"]]
                    slope_data["task"] = [trial_info["task"]]
                    ...
                    vel_motion_table, vel_emg_table = FigPlot.plot_arm_angular(motion_list[num], str(folder_name + save_place),
                                                                               Elbow_AngVel, Wrist_AngVel, emg_iMVC,
                                                                               joint_threshold, cal_method='vel',
                                                                               mot_emg_ratio=emg_fs/motion_info['frame_rate'])
                    acc_motion_table, acc_emg_table = FigPlot.plot_arm_angular(motion_list[num], str(folder_name + save_place),
                                                                               Elbow_AngAcc, Wrist_AngAcc, emg_iMVC,
                                                                               joint_threshold, cal_method='acc',
                                                                               mot_emg_ratio=emg_fs/motion_info['frame_rate'])
