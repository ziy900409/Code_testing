...
for folder_name in vicon_folder:
    ...
    for num in range(len(motion_list)):
        ...
        # 結束索引是開始後加15秒
        task_end = task_start + int(motion_info['frame_rate']*20)
        ...
        # import pdb
        # pdb.set_trace()
        vel_motion_table, vel_emg_table = FigPlot.plot_arm_angular(motion_list[num], folder_name,
                                                                   Elbow_AngVel, Wrist_AngVel, emg_iMVC,
                                                                   joint_threshold, cal_method='vel')
        acc_motion_table, acc_emg_table = FigPlot.plot_arm_angular(motion_list[num], folder_name,
...
with pd.ExcelWriter(folder_path + "4. Statistics\\" + save_place + "All_" + task_name + "_vicon_motion+emg_" + formatted_date + ".xlsx",
    ...
    all_emg_data_table.to_excel(
        writer, sheet_name='arm_motion', index=False, header=True)
    all_motion_data_table.to_excel(
        writer, sheet_name='emg_table', index=False, header=True)
    ...
    for stage_num in range(len(stage_file["Motion_File_C3D"])):
        for num in range(len(motion_list)):
            if str(stage_file["Motion_File_C3D"][stage_num]) in motion_list[num]:
                ...
                motion_info, motion_data, analog_info, analog_data, np_motion_data = func.read_c3d(motion_list[num],
                                                                                                   method='vicon')
                emg_data = pd.read_csv(str(folder_path + EMG_folder + folder_name + "\\" + stage_file['EMG_File'][stage_num]),
                                           encoding='UTF-8')
                ...
                # 取代 LABELS 中的 subject name
                new_columns_list = [
                    s.replace(str(folder_name + ':'), '') for s in motion_info["LABELS"]]
                ...
                emg_task_start = int(task_start * emg_fs /
