# -*- coding: utf-8 -*-
"""
Created on Tue Mar 26 15:44:44 2024
...
def EMG_processing(raw_data_path, smoothing="lowpass"):
    ...
    # 1.3.-------------創建儲存EMG data的矩陣------------
    # bandpass filter used in signal
    bandpass_filtered_data = pd.DataFrame(np.zeros([int(data_len//Fs/down_freq), len(num_columns)]),
                            columns=raw_data.iloc[:, num_columns].columns)
    notch_filtered_data = pd.DataFrame(np.zeros([int(data_len//Fs/down_freq), len(num_columns)]),
                            columns=raw_data.iloc[:, num_columns].columns)
    lowpass_filtered_data = pd.DataFrame(np.zeros([int(data_len//Fs/down_freq), len(num_columns)]),
                            columns=raw_data.iloc[:, num_columns].columns)
    ...
    for col in range(len(num_columns)):
        if '.csv' in raw_data_path:
            ...
            isnan = np.where(np.isnan(raw_data.iloc[:(np.shape(raw_data)[0] - data_len), num_columns[col]]))
        ...
        notch_filtered_data.iloc[:, col] = notch_filtered
