...
#         onset_z = detect_onset(filted_motion.loc[motion_start:motion_start+int(motion_info['frame_rate']*4), 'R.M.Finger1_z'].values,
#                                1.01*np.mean(filted_motion.loc[motion_start:motion_start+18, 'R.M.Finger1_z'].values),
#                                n_above=10, n_below=0, show=True)
...
#                       "rx") 
#         axes[2].set_title('R.M.Finger1_z')  # 设置子图标题
#         # 绘制第四个子图
...
data_store = pd.DataFrame({},
                          columns=['file_name', "direction", "max_value", "times",
                                   'Extensor carpi radialis_RMS', 'Flexor Carpi Radialis_RMS', 'Triceps_RMS',
                                   'Extensor carpi ulnaris_RMS', '1st. dorsal interosseous_RMS',
                                   'Abductor digiti quinti_RMS', 'Extensor Indicis_RMS', 'Biceps_RMS'])
...
for folder in rowdata_folder_list:
    ...
    for file_path in range(len(liftingShot_list)):  
        ...
        if 'EC2 Wight:R.M.Finger1_x' in motion_data.columns:
            ...
            plt.close()
            motion_start = int((onset_analog[0, 0] + 5*analog_info['frame_rate']) / 10)
        elif 'RMD1_x' in motion_data.columns:
            print(1)
            motion_data.rename(columns=c3d_recolumns_vicon, inplace=True)
            motion_start = int(5 * motion_info['frame_rate'])
        ...
        for i in range(np.shape(motion_data)[1]-1):
            filted_motion.iloc[:, i+1] = signal.sosfiltfilt(lowpass_sos,
                                                    motion_data.iloc[:, i+1].values)
        vel_MFinger1_z = abs(filted_motion.loc[1:, 'R.M.Finger1_z'].values - \
            filted_motion.loc[:len(filted_motion)-2, 'R.M.Finger1_z'].values)
        
        
        # onset_z = detect_onset(filted_motion[motion_start:motion_start+int(motion_info['frame_rate']*4), 'R.M.Finger1_z'].values,
        #                        1.02*np.mean(filted_motion.loc[motion_start:motion_start+18, 'R.M.Finger1_z'].values),
        #                        n_above=10, n_below=0, show=True)
        
        onset_z = detect_onset(vel_MFinger1_z[motion_start:motion_start+int(motion_info['frame_rate']*4)],
                               0.1,
                               n_above=10, n_below=5, show=False)
        # plt.close()
        
        mark_circle = onset_z[0, 0] + motion_start
        # if len(onset_z) < 3:
        #     for i in range(len(onset_z)):
        #         mark_circle.append(onset_z[i, 0] + motion_start)
        # else:
        #     for i in range(3):
        #         mark_circle.append(onset_z[i, 0] + motion_start)
        ...
        marker_name = ['R.M.Finger1_x', 'R.M.Finger1_y', 'R.M.Finger1_z']
        # 创建一个包含两个子图的图形，并指定子图的布局
        fig, axes = plt.subplots(4, 1, figsize=(12, 6))  # 2 行 1 列，每个子图的大小为 (8, 6)
        # 绘制第一个子图
        axes[0].plot(filted_motion.loc[:, 'Frame'].values,
        ...
        # 繪製第四個子圖
        axes[3].plot(filted_motion.loc[:len(filted_motion)-2, 'Frame'].values,
                     vel_MFinger1_z,
                     color='blue')  # 假设 data2 是一个 Series 或 DataFrame
        axes[3].plot(filted_motion.loc[mark_circle, 'Frame'],
                     vel_MFinger1_z[mark_circle],
                     marker = 'o', ms = 10, mec='r', mfc='none')
        axes[3].set_title('Vel R.M.Finger1_z')  # 设置子图标题
        # 繪製圓圈
        for subfig in range(len(axes)-1):
            axes[subfig].plot(filted_motion.loc[mark_circle, 'Frame'],
                              filted_motion.loc[mark_circle, marker_name[subfig]],
                              marker = 'o', ms = 10, mec='r', mfc='none')
        # 添加整体标题
        fig.suptitle(filename)  # 设置整体标题
        # 调整子图之间的间距
        plt.tight_layout()
        plt.savefig(str(fig_sace_path + filename + "_motion_liftZ.jpg"),
                    dpi=100)
        # 显示图形
        # plt.show()
        # plt.ioff()
        ...
        if folder not in ["S5", "S6", "S7"]:
            ...
            # emg_max_idx = int(first_max_idx/motion_info['frame_rate']*emg_smaple_rate - \
                # pa_start_onset[0][0]/analog_info['frame_rate']*emg_smaple_rate)
            emg_mark_idx = []
            # for i in range(len(mark_circle)):
            emg_mark_idx.append(int(mark_circle/motion_info['frame_rate']*emg_smaple_rate - \
                    pa_start_onset[0][0]/analog_info['frame_rate']*emg_smaple_rate)) 
        ...
        for i in range(np.shape(processing_data)[1]-1):
            ...
            axs[x, y].set_title(processing_data.columns[i+1], fontsize=16)
            # 在X軸指定的範圍上加上色塊
            # for marker in range(len(emg_mark_idx)):
            axs[x, y].axvspan(emg_mark_idx[0]/emg_smaple_rate - 0.05,
                              emg_mark_idx[0]/emg_smaple_rate + 0.05,
                              color='red', alpha=0.3)  # alpha是色塊的透明度
        
            # 設定科學符號 : 小數點後幾位數
            axs[x, y].ticklabel_format(axis='y', style = 'scientific', scilimits = (-2, 2))
        # 設定整張圖片之參數
        plt.suptitle(filename + str("_liftZ"), fontsize = 16)
        plt.tight_layout()
        fig.add_subplot(111, frameon=False)
        # hide tick and tick label of the big axes
        plt.tick_params(labelcolor='none', top=False, bottom=False, left=False, right=False)
        ...
        plt.xlabel("time (second)", fontsize = 14)
        plt.ylabel("Voltage (V)", fontsize = 14)
        plt.savefig(str(fig_svae_path + '\\' + "liftZ_" + filename + ".jpg"),
                    dpi=200, bbox_inches = "tight")
        ...
        # for idx in range(len(emg_mark_idx)):
        for column in range(np.shape(emg_iMVC)[1]-1):
            rms_data.iloc[:, column] =   np.sqrt(np.sum((emg_iMVC.iloc[int(emg_mark_idx-emg_smaple_rate*0.2):\
                                                                       int(emg_mark_idx+emg_smaple_rate*0.05), column+1])**2)
                                                  /len(emg_iMVC.iloc[int(emg_mark_idx-emg_smaple_rate*0.2):\
                                                                      int(emg_mark_idx+emg_smaple_rate*0.05), column+1]))
            #         # 將資料儲存至矩陣
            # 將資料儲存至矩陣
        add_data = pd.DataFrame({"file_name":filename,
                                 "direction":vel_direc,
                                 "times":0,
                                 "max_value": filted_motion.loc[mark_circle, 'R.M.Finger1_z'],
                                 'Extensor carpi radialis_RMS':rms_data.iloc[:, 0],
                                 'Flexor Carpi Radialis_RMS':rms_data.iloc[:, 1],
                                 'Triceps_RMS':rms_data.iloc[:, 2],
                                 'Extensor carpi ulnaris_RMS':rms_data.iloc[:, 3],
                                ...
                                index=[0])
        data_store = pd.concat([add_data, data_store], ignore_index=True)
    del processing_data, bandpass_filtered_data, rms_data, add_data, emg_iMVC, motion_info, motion_data, analog_info, analog_data, np_motion_data
    gc.collect(generation=2)
path = r"D:\BenQ_Project\01_UR_lab\09_ZowieAllSeries\5. Statistics\\"
file_name = "LiftingShot_LiftZ_" + formatted_date + ".xlsx"
data_store.to_excel(path + file_name,
                    sheet_name='Sheet1', index=False, header=True)
