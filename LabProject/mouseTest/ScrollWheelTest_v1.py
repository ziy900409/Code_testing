...
temp_params = {}
if "ScrollWheelTest_temp.txt" in os.listdir(current_path):
    print(0)
    temp_txt_path = current_path + "\\" + "ScrollWheelTest_temp.txt"
...
entry_width_range.insert(0, "[50, 150]")  # 設置預設文字
...
# params ={"circle_radius": [150, 250],
#          "target_y": [1000, 800]}

all_combinations = []
for i in params["circle_radius"]:
    for ii in params["target_y"]:
        all_combinations.append({'circle_radius': i,
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            wheel_data.append((Participant, Condition, Block, # 受試者, 條件, 第幾次測試
                               ...
                               "WHEELDATA", pygame.time.get_ticks(), target_y))
            # # 记录目标物的位置
            ...
            if distance <= circle_radius and ButtonDown:
                ...
                if current_level < len(all_combinations):
                    mouse_click_events.append((Participant, Condition, Block, # 受試者, 條件, 第幾次測試
                                               (current_level + 1), # sequemce
                                               circle_radius, all_combinations[current_level]["target_y"], # ampltude, width
                                               "MOUSEBUTTONDOWN", pygame.time.get_ticks(), event.pos[1])) # event, mouse position, time
                    circle_radius = all_combinations[current_level]["circle_radius"]
                    target_y = all_combinations[current_level]["target_y"]
...
#                       'Radius', 'Distance',
#                       'Event', 'time', 'Pos_y'])
#     for part, cond, blo, sec, trial, amp, wid, t, pos in wheel_data:
#         writer.writerow([part, cond, blo, sec, trial, amp, wid, 'MOUSEPOS', time, pos])
#     for part, cond, blo, sec, trial, amp, wid, event, t , pos in mouse_click_events:
#         writer.writerow([part, cond, blo, sec, trial, amp, wid, event, time, pos])
