...
# 主循環
running = True
while running and test_count < max_tests:
    screen.fill(WHITE)
    
    # 顯示 task 資訊
    tests_info = font.render(Participant,
                              True, (0, 0, 0))
    tests_para = font.render(Condition,
                            True, (0, 0, 0))
    tests_num = font.render(test_count,
                            True, (0, 0, 0))
    screen.blit(tests_info, (20, 20))
    screen.blit(tests_para, (20, 60))
    screen.blit(tests_num, (20, 100))
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            running = False
        elif event.type == pygame.MOUSEBUTTONDOWN and current_color == GREEN:
...
df.to_excel(r'D:\BenQ_Project\FittsDragDropTest\ReactionTimeTest\reaction_times.xlsx', index=False)
