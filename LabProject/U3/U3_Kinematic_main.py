# -*- coding: utf-8 -*-
"""
Created on Fri Jul 21 14:10:21 2023
...
import U3_EMG_function_v1 as emg
...
'''
# 儲存資料
fitts_data = pd.DataFrame({'file_name':[], 'Fitts_end':[]})
# 在不同的受試者資料夾下執行
for folder_name in motion_folder_list:
    ...
    c3d_list = func.Read_File(motion_path + folder_name,
                              ".c3d")
    # 找出一樣的 motion 檔名與 all table
    for num in range(len(c3d_list)):
        for i in range(len(all_table['c3d'])):
            if "Fitts" in c3d_list[num] and \
                c3d_list[num].split('\\')[-1] == all_table['c3d'][i]:
                print(c3d_list[num])
                print(all_table['c3d'][i])
                file_name = all_table['Fitts_SD2'][i].split('.')[0]
                file_name = file_name.split('-')
                # 找出相應符合的 Fitts file
                total_time = 0
                for ii in range(len(Fitts_table["Condition"])):
                    if file_name[2] == Fitts_table["Condition"][ii] and \
                        file_name[-1] == Fitts_table["Block"][ii]:
                        print(Fitts_table["Condition"][ii], Fitts_table["Block"][ii])
                        total_time += Fitts_table.loc[ii, 'MT(ms)']
                # 找出
                all_time = total_time*15 + 8000
                fitts_data = pd.concat([fitts_data, 
                                        pd.DataFrame({'file_name':all_table['c3d'][i], 'Fitts_end':all_time}, index=[0])],
                                       ignore_index=True)
...
for folder_name in motion_folder_list:
    ...
    all_emg_data_table = pd.DataFrame({}, columns = ['檔名', 'direction', 'axis', 'type',
                                                 ...
                                                 'ExtR-mean', 'ExtR-max', 'ExtU-mean', 'ExtU-max',
                                                 'FlexR-mean', 'FlexR-max', 'Biceps-mean', 'Biceps-max'])
    all_z_emg_data_table = pd.DataFrame({}, columns = ['檔名', 'direction', 'axis', 'type',
                                                 'Dor1st-mean', 'Dor1st-max', 'Dor3rd-mean', 'Dor3rd-max',
                                                 'abduciton-mean', 'abduciton-max', 'indict-mean', 'indict-max',
                                                 'ExtR-mean', 'ExtR-max', 'ExtU-mean', 'ExtU-max',
                                                 'FlexR-mean', 'FlexR-max', 'Biceps-mean', 'Biceps-max'])
    ...
    z_motion_table = pd.DataFrame({}, columns = ['檔名', 'vel_mean', 'vel_max', 'acc_mean', 'acc_max'])
    hand_angle_table = pd.DataFrame({}, columns=["CMP1",
                                                 "CMP2", "PIP2",
                                                 "CMP3",
                                                 "CMP4", "CMP5"])
    # ------第一次loop先計算tpose的問題------------------------------------------------
    ...
    for num in range(len(c3d_list)):
        for i in range(len(all_table['c3d'])):
            if c3d_list[num].split('\\')[-1] == all_table['c3d'][i]:
                # 1.2.1. ---------找到Analog data 中 trigger 的起始時間-----------------------------
                if "Blink" in c3d_list[num]:
                    print(c3d_list[num])
                    # print(all_table['EMG'][i])
                    # 1. read c3d file
                    motion_info, motion_data, _, analog_data, np_motion_data = func.read_c3d(c3d_list[num])
                    # 2. find peak with threshold (please parameter setting)
                    peaks = detect_onset(analog_data.loc[:, "trigger1"], np.std(analog_data.loc[:, "trigger1"][:18]),
                                         n_above=10, n_below=0, show=True)
                    # 3. 找出 analog, motion 兩個時間最接近的 frame, 並定義 start index
                    ...
                    emg_data = pd.read_csv(str(emg_folder_path + folder_name + "\\motion\\" + all_table['EMG'][i]),
                                           encoding='UTF-8')
                    processing_data, _ = emg.EMG_processing(emg_data, smoothing="lowpass")
                    # --------------------計算 iMVC---------------------------
                    emg_iMVC = pd.DataFrame(np.zeros(np.shape(processing_data)),
                                            columns=processing_data.columns)
                    emg_iMVC.iloc[:, 0] = processing_data.iloc[:, 0].values
                    ...
                    # 裁切 EMG data
                    # 因為採樣頻率是motion data的十倍, 並且從trigger開始八秒之後才算
                    trun_emg_iMVC = emg_iMVC.iloc[int(motion_info["frame_rate"]*10*8):int(motion_info["frame_rate"]*10*(8+52)) ,:]
   
                    # 計算大臂, 小臂, 手掌隨時間變化的坐標系
                    # new_motion_info:更新LABELS, new_motion_data: 去掉Virtual marker
                    ArmCoord, ForearmCoord, HandCoord,  new_motion_info, new_motion_data = pro.UpperExtremty_coord(trun_motion, motion_info, p1_all)
                    del  motion_info, motion_data, analog_data, np_motion_data, processing_data
                    # ---------------------------------------------------------------------------
                    # 計算手指的關節角度: 只計算食、中指以及大拇指.
                    ...
                    # 尋找 M.Finger1的 z-axis 向上的行為
                    tem_z_motion_table, z_emg_data_table = FigPlot.find_Z_peak(c3d_list[num],
                                                                         new_motion_data, new_motion_info, joint_threshold,
                                                                         trun_emg_iMVC)
                    z_motion_table = pd.concat([z_motion_table, tem_z_motion_table])
                    all_z_emg_data_table = pd.concat([all_z_emg_data_table, z_emg_data_table])
                    
    motion_file_name = r"E:\BenQ_Project\U3\09_Results\\" + folder_name + "_Blink_motion_table.xlsx"
    emg_file_name = r"E:\BenQ_Project\U3\09_Results\\" + folder_name + "_Blink_emg_table.xlsx"
    MedFreq_file_name = r"E:\BenQ_Project\U3\09_Results\\" + folder_name + "_Blink_MedFreq_table.xlsx"
    z_motion_file_name = r"E:\BenQ_Project\U3\09_Results\\" + folder_name + "_Blink_Zmotion_table.xlsx"
    z_emg_file_name = r"E:\BenQ_Project\U3\09_Results\\" + folder_name + "_Blink_emg_table.xlsx"
    hand_angle_table_name =  r"E:\BenQ_Project\U3\09_Results\\" + folder_name + "_Blink_FingerAngle_table.xlsx"
    # 將資料匯出至excel
    # pd.DataFrame(all_motion_data_table).to_excel(motion_file_name, sheet_name='Sheet1', index=False, header=True)
    # pd.DataFrame(all_emg_data_table).to_excel(emg_file_name, sheet_name='Sheet1', index=False, header=True)
    # pd.DataFrame(all_median_freq_table).to_excel(MedFreq_file_name, sheet_name='Sheet1', index=False, header=True)  
    # pd.DataFrame(z_motion_table).to_excel(z_motion_file_name, sheet_name='Sheet1', index=False, header=True)
    pd.DataFrame(all_z_emg_data_table).to_excel(z_emg_file_name, sheet_name='Sheet1', index=False, header=True)
    pd.DataFrame(hand_angle_table).to_excel(hand_angle_table_name, sheet_name='Sheet1', index=False, header=True)            
0# %% 抓取時間
from io import TextIOWrapper

# %% 抓取
motion_info, motion_data, analog_info, analog_data, np_motion_data = func.read_c3d(r"E:\Motion Analysis\U3 Research\S01\S01_Fitts1_4.c3d")
...
# reader.SetFilename(r"E:\Motion Analysis\U3 Research\S01_Fitts1_1.trc")
# reader.Update()
# acq = reader.GetOutput()

motion_info, motion_data, analog_info, FP_data = func.read_c3d(r"E:\Motion Analysis\U3 Research\S01\S01_Spider4_4.c3d")
...
def update(frame):
    # 生成新的随机数据
    new_x = np.random.rand(n_points)
    new_y = np.random.rand(n_points)
