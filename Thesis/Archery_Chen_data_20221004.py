...
to deal RMS for moving average for 100 ms
    ...
    for i in range(len(shooting_file_list)):
        for ii in range(len(staging_data['FileName'])):
            if shooting_file_list[i] == staging_data['FileName_rms'][ii].split('\\')[-1]:
                if np.isnan(staging_data['Clicker'][ii]) != 1:
                    release_time = int((staging_data["Clicker"][ii] - staging_data['Trigger'][ii])*10)
                    print(release_time)
                    ...
                    shooting_EMG = shooting_data.iloc[release_time - 5000:release_time + 2000, 1:]    
                    # calculate iMVC data
                    ...
                    save_iMVC_name = save_file_path + '\\韻律\\' + 'iMVC_' + shooting_file_list[i]
            ...
            for ii in range(len(staging_data['FileName'])):
                if fatigue_file_list[i] == staging_data['FileName_rms'][ii].split('\\')[-1]:
                    if np.isnan(staging_data['Clicker'][ii]) != 1:
                        ...
                        # trunkcate specific period
                        fatigue_EMG = fatigue_data.iloc[release_time - 5000:release_time + 2000, 1:]    
                        # calculate iMVC data
                        fatigue_iMVC = np.divide(fatigue_EMG, MVC_value)*100
                        fatigue_iMVC.insert(0, 'time', fatigue_data.iloc[:,0])
                        # writting iMVC data in a EXCEL
                        save_iMVC_name = save_file_path + '\\機械\\' + 'iMVC_' + fatigue_file_list[i]
