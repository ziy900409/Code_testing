...
for ii in data_list:
    ...
    res_data = force_z.iloc[::-1] # 去除從後倒數回來的0值
    a = (res_data.values > 200).argmax(axis=0) # 先反轉矩陣，再用argmax，返回第一個boolean最大值
    cut_force_z = pd.DataFrame(force_z.iloc[:int(9999-a-1), 0])
    ...
    # 計算騰空時間
    # 找出所有數值小於5N的位置
    jump_time = ((cut_force_z.values < 5).sum())/sampling_rate
    # 計算跳躍高度 (m)
    ...
    load_factor = (cut_force_z.iloc[peak_force] - cut_force_z.iloc[touch_time]) \
        /((peak_force-touch_time)/sampling_rate)
    ...
    acc_curve_x = time_span[acc_period[0][0]:acc_period[0][1]]
    ...
    slove = op.fsolve(diff_acc_curve_poly,
                      [time_span[acc_period[0][0]], time_span[acc_period[0][1]]])
    ...
    explosive = (cut_force_z.iloc[int((acc_curve_x[0] + ind[0]/sampling_rate)*sampling_rate), 0] \
                 - cut_force_z.iloc[peak_down_period, 0]) / \
        (int((acc_curve_x[0] + ind[0]/sampling_rate)*sampling_rate) \
         - peak_down_period) * sampling_rate
    # 測試曲線擬合程度: 最小二乘擬合
    x = []
    y = range(2, 200, 2)
    ...
    plt.figure(1)
    plt.plot(time_span, cut_force_z.iloc[:, 0])
    plt.plot(peak_down_period/sampling_rate, cut_force_z.iloc[peak_down_period, 0], 'o')
    plt.plot(peak_force/sampling_rate, cut_force_z.iloc[peak_force, 0], 'o')
    plt.plot(touch_time/sampling_rate, cut_force_z.iloc[touch_time, 0], 'o')
    plt.plot(acc_period[0]/sampling_rate, cut_force_z.iloc[acc_period[0], 0], 'o')
    plt.plot((acc_curve_x[0] + ind[0]/sampling_rate),
             cut_force_z.iloc[int((acc_curve_x[0] + ind[0]/sampling_rate)*sampling_rate), 0], 'o')
    ...
    add_ForcePlate_data = pd.DataFrame({
                                    ...
                                    'peak_push_time': [int((acc_curve_x[0] + ind[0]/sampling_rate)*sampling_rate)],
