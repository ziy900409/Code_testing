...
csv_recolumns_name = {'R BICEPS BRACHII: EMG 1': 'Briceps Brachii',
                      'R TRICEPS BRACHII: EMG 2': 'Triceps Brachii',
                      'R EXTENSOR CARPI RADIALIS: EMG.A 4': 'Extensor Carpi Radialis',
                      'R EXTENSOR CARPI RADIALIS: EMG.B 4': 'Extensor Carpi Ulnaris',
                      'R EXTENSOR CARPI RADIALIS: EMG.C 4': 'Flexor Carpi Radialis',
                      'R EXTENSOR CARPI RADIALIS: EMG.D 4': 'Flexor Carpi Ulnaris'}
...
for i in range(len(rowdata_folder_list)):
    ...
    for MVC_path in MVC_list:
        ...
        # 將肌肉名稱換掉
        bandpass_filtered_data.rename(columns=csv_recolumns_name, inplace=True)
        processing_data.rename(columns=csv_recolumns_name, inplace=True)
...
anc_time = pd.DataFrame({"file_name": [],
                         "idx": []})
emg_data_table = pd.DataFrame({}, columns = ['task','trial', 'time', 'Briceps Brachii','Triceps Brachii', 'Extensor Carpi Radialis',
...
for iii in range(len(rowdata_folder_list)):
    ...
    Staging_file = pd.read_excel(r"E:\Hsin\NTSU_lab\Baseball\motion分期肌電用_20240420.xlsx", sheet_name=rowdata_folder_list[iii])
    ...
    for motion_path in motion_file_list:
        ...
        for emg_name in range(len(Staging_file['EMG檔案'])):
            if list_filename == Staging_file['EMG檔案'][emg_name]:
                ...
                plt.savefig(save, dpi=200, bbox_inches = "tight")
                plt.show()
emg_data_table.rename(columns=csv_recolumns_name, inplace=True)
