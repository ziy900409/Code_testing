# -*- coding: utf-8 -*-
"""
Created on Wed Apr 26 08:46:56 2023
...
def plot_plot(data, savepath, filename, filter_type, annotation=None):
    ...
    for i in range(np.shape(data)[1]-1):
        ...
        axs[x, y].ticklabel_format(axis='y', style = 'scientific', scilimits = (-2, 2))
        if annotation:
            axs[x, y].axvline(x=annotation[0], color = 'darkslategray', linewidth=1, linestyle = '--')
            axs[x, y].axvline(x=annotation[1], color = 'darkslategray', linewidth=1, linestyle = '--')
...
# plt.stem(freq[0:int(N/2)], 2.0/N * abs(X[0:int(N/2)]))
# plt.plot(freq[0:int(N/2)], 2.0/N * abs(X[0:int(N/2)]))
# plt.xlabel('Freq (Hz)')
# plt.ylabel('FFT Amplitude |X(freq)|')


# plt.subplot(122)
# plt.plot(freq, ifft(X), 'r')
# plt.xlabel('Time (s)')
# plt.ylabel('Amplitude')
# plt.tight_layout()
# plt.show()
...
# plt.subplot(122)
# plt.plot(freq, fftpack.ifft(X), 'r')
# plt.xlabel('Time (s)')
# plt.ylabel('Amplitude')
# plt.tight_layout()
# plt.show()
